...
for file in ` ls $TOP_DIR/*16901*.out.bam `
do
	...
	cat <( cat ${bfname}.sam ) \
		...
		| samtools sort -n - | samtools view -bS - > ${bfname}.bam
	snaptools snap-pre \
...
do
	snaptools snap-add-pmat \
		--snap-file=${file} \
		--peak-file ../data/peaks/peaks_cluster.combined.bed
done
